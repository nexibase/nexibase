...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    const redirectUrl = `${baseUrl}/auction?error=payment_failed&message=${encodeURIComponent(message || '결제에 실패했습니다.')}`
  ...
  if (orderNo) {
    const redirectUrl = `${baseUrl}/auction?paid=true&orderNo=${orderNo}`
    return createRedirectHtml(redirectUrl)
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (authResult.resultCode === '0000') {
      ...
      // 경매 완료 페이지로 리다이렉트
      const successUrl = auctionId
        ? `/auction/${auctionId}?paid=true`
        : `/auction?paid=true`
